...
apply_heat_pipe_glow = function(layer)
  ...
  return { layers = { layer, light_layer } }
end
  ...
  for key, t in pairs(data) do
    if t.empty then
      ...
      for i = 1, (t.variations or 1) do
        local sprite = func {
          priority = 'extra-high',
          filename = path .. name_prefix .. '-' .. (t.name or string.gsub(key, '_', '-')) ..
              (t.ommit_number and '.png' or ('-' .. tostring(i) .. '.png')),
          width = (t.width or 32),
          height = (t.height or 32),
          shift = t.shift,
          hr_version = {
            priority = 'extra-high',
            filename = path .. 'hr-' .. name_prefix .. '-' .. (t.name or string.gsub(key, '_', '-')) ..
                (t.ommit_number and '.png' or ('-' .. tostring(i) .. '.png')),
